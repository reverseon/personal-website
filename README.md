...
The renderer has been modularized for better maintainability:
- **`main.rb`**: Entry point that orchestrates the rendering process.
- **`loaders/`**: Handles loading and parsing metadata from markdown posts (`post_loader.rb`) and YAML thought files (`thought_loader.rb`).
- **`renderers/`**: Contains specialized classes for rendering different page types (`post_renderer.rb`, `thought_renderer.rb`, `profile_renderer.rb`, `error_renderer.rb`).
- **`utils/`**: Utility modules for pagination (`paginator.rb`) and HTML formatting/post-processing (`html_formatter.rb`).
...
- `css/` - Cascading Style Sheets.
  - `global/` - Global styles and design system tokens (e.g., `main.css`).
  - `components/` - Component-specific styles (e.g., `post.css`, `train-of-thoughts.css`).
- `js/` - JavaScript files.
  - `global/` - Global scripts (e.g., `main.js`, `share-btn.js` for toast notifications).
...
- Wrapped in `<span class="rg">` for grouped hover behavior (readings revealed on hover).

**Footnotes**:
```
<!--Footnote Your note here -->
```
- Converts to `<aside class="footnote">` elements with a "Footnote" label.
- Supports multiline notes and distinct styling from the main body.
